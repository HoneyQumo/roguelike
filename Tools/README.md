...
```bash
python Tools/Project/sync_filters.py
```

MSBuild собирает по `.vcxproj`, а дерево в Solution Explorer рисуется по `.vcxproj.filters`. Файл легко добавить в первый и забыть про второй - тогда он собирается, но в дереве его нет. Скрипт пересобирает дерево по сборке, а `ProjectTreeTest` говорит, когда пора его запустить.
